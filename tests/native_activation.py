...
Run: python3 tests/native_activation.py [--binary /path/to/lilt]
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='lilt-activation-test-') as directory:
        temp = Path(directory)
        for child in ('home', 'data/lilt/models', 'config/ptt', 'cache', 'runtime', 'services'):
        ...
        # Reach capture setup with a placeholder; the private audio address is absent.
        (temp / 'data/lilt/models/ggml-medium.en-q5_0.bin').touch()
...
''')
        ...
        env.update(
            ...
            LILT_TEST_ISOLATED='1', LILT_TEST_BINARY=str(binary),
        )
